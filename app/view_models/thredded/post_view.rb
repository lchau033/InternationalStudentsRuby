...
module Thredded
  ...
  class PostView
    ...
    def current_user_upvoted?(current_user)
      @current_user_upvoted ||= !Upvote.where(:user_id => current_user, :post_id => @post.id).blank?
    end

    def current_user_downvoted?(current_user)
      @current_user_downvoted ||= !Downvote.where(:user_id => current_user, :post_id => @post.id).blank?
    end
